...
//     // API Base URL - تأكد من الرابط الصحيح
//     const API_BASE_URL = 'http://192.168.3.11:5000/api';
...
//                 // Fetch specializations - المسار المعدل
//                 const specializationsData = await fetchApiData('/catalog/admin/specializations');
//                 console.log('Specializations API response:', specializationsData);
                
//                 if (specializationsData.success) {
//                     // التأكد من أن البيانات تأتي من المكان الصحيح
//                     const specializationsList = specializationsData.data.data || specializationsData.data || [];
//                     const mappedSpecializations = specializationsList.map(spec => ({
//                         ...spec,
...
//                 } else {
//                     showToast('تحذير', 'لم يتم جلب الاختصاصات بنجاح', 'destructive');
//                 }
                
//                 // Fetch instructors - تأكد من المسار الصحيح
//                 try {
//                     const instructorsData = await fetchApiData('/instructors');
//                     if (instructorsData.success) {
//                         const instructorsList = instructorsData.data.data || instructorsData.data || [];
//                         setInstructors(instructorsList.map(inst => ({
//                             ...inst,
...
//                     showToast('تحذير', 'فشل في جلب بيانات المدرسين', 'destructive');
//                 }
                
//                 // Fetch subjects
//                 try {
//                     const subjectsData = await fetchApiData('/subjects');
//                     if (subjectsData.success) {
//                         const subjectsList = subjectsData.data.data || subjectsData.data || [];
//                         setSubjects(subjectsList.map(sub => ({
//                             ...sub,
...
//                     showToast('تحذير', 'فشل في جلب بيانات المواد', 'destructive');
//                 }
                
//                 // Fetch courses
//                 try {
//                     const coursesData = await fetchApiData('/courses');
...
//                     showToast('تحذير', 'فشل في جلب بيانات الدورات', 'destructive');
//                 }
                
//                 // Fetch course levels
//                 try {
//                     const levelsData = await fetchApiData('/course-levels');
//                     if (levelsData.success) {
//                         const levelsList = levelsData.data.data || levelsData.data || [];
//                         setCourseLevels(levelsList.map(level => ({
//                             ...level,
...
//                     showToast('تحذير', 'فشل في جلب بيانات المستويات', 'destructive');
//                 }
                
//                 // Fetch lessons
//                 try {
//                     const lessonsData = await fetchApiData('/lessons');
//                     if (lessonsData.success) {
//                         const lessonsList = lessonsData.data.data || lessonsData.data || [];
//                         setLessons(lessonsList.map(lesson => ({
//                             ...lesson,
...
//                     showToast('تحذير', 'فشل في جلب بيانات الدروس', 'destructive');
//                 }
                
//             } catch (error) {
//                 console.error('Error fetching data:', error);
//                 showToast('خطأ', 'فشل في جلب البيانات من الخادم', 'destructive');
//             } finally {
//                 setLoading(false);
//             }
//         };
...
//         addCourse: false, editCourse: false, addLevel: false, addLesson: false, 
//         editLesson: false, addInstructor: false, editInstructor: false, 
//         addSpecialization: false, editSpecialization: false, addSubject: false, 
//         editSubject: false 
//     });
...
//         specialization: { name: "", isActive: true },
...
//     const getSubjectsBySpecialization = (specializationId) => 
//         subjects.filter(s => s.specializationId === parseInt(specializationId));
    
//     const getSubjectsByInstructor = (instructorId) => 
//         subjects.filter(s => s.instructorId === parseInt(instructorId));
    
//     const getCourseLevels = (courseId) => 
//         courseLevels.filter(l => l.courseId === courseId).sort((a, b) => a.order - b.order);

//     // --- دالة لإضافة بيانات جديدة عبر API ---
//     const handleAddWithApi = async (type, data, successMessage) => {
//         try {
...
//             if (result.success) {
//                 showToast("تمت الإضافة", successMessage);
//                 // إعادة تحميل البيانات أو إضافة العنصر مباشرة للstate
//                 return result.data;
//             } else {
...
//             showToast("خطأ", `فشل في إضافة ${type}`, "destructive");
//             throw error;
//         }
//     };
...
//         try {
//             // هنا يمكنك استخدام handleAddWithApi بدلاً من الإضافة المحلية
//             const newItem = { 
//                 id: Date.now(), 
//                 ...data, 
//                 isActive: true 
...
//         const updater = (prev) => prev.map(item => (item.id === editingItem.id ? { ...item, ...data } : item));
...
//         const updater = (prev) => prev.map(item => (item.id === id ? { ...item, isActive: !item.isActive } : item));
//         if (type === 'specialization') setSpecializations(updater);
//         if (type === 'instructor') setInstructors(updater);
//         if (type === 'subject') setSubjects(updater);
//         if (type === 'course') setCourses(updater);
...
//                     <TabsTrigger value="taxonomy" className="cursor-pointer">التصنيف (الاختصاص، المدرس، المادة)</TabsTrigger>
//                     <TabsTrigger value="courses" className="cursor-pointer">الدورات والدروس</TabsTrigger>
//                 </TabsList>
...
//                                     <DialogTrigger asChild><Button size="sm">إضافة<Plus className="w-4 h-4 ml-1" /> </Button></DialogTrigger>
//                                     <DialogContent>
//                                         <DialogHeader className="items-center"><DialogTitle>إضافة اختصاص جديد</DialogTitle></DialogHeader>
...
//                                             <Checkbox id="isActive" checked={form.specialization.isActive} onCheckedChange={(checked) => handleCourseFormChange('isFree', checked)} />
...
//                                 {specializationsArray.map(item => (
//                                     <div key={item.id} className="flex items-center justify-between p-2 border rounded">
//                                         <span>{item.name}</span>
//                                         <div className="flex gap-1">
//                                             <div className="flex gap-2 mt-2">
//                                                 <Badge variant={item.isActive ? "default" : "secondary"}>{item.isActive ? "نشط" : "معطل"}</Badge>
//                                             </div>
...
//                 <DialogContent>
//                     <DialogHeader className="items-center"><DialogTitle>تعديل الاختصاص</DialogTitle></DialogHeader>
//                     <Input value={form.specialization.name || ''} onChange={(e) => handleFormChange('specialization', 'name', e.target.value)} />
//                     <div className="flex items-center space-x-2">
//                         <Checkbox id="isActive" checked={form.specialization.isActive} onCheckedChange={(checked) => handleCourseFormChange('isActive', checked)} />
//                         <label htmlFor="isActive">مفعل</label>
//                     </div>
